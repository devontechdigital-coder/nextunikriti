...
import { Container, Row, Col, Card, Form, Button, Spinner, Alert, ListGroup, Badge, Table, Modal } from 'react-bootstrap';
import { 
...
import { 
  FaUserGraduate, FaSave, FaPlus, FaTrash, FaSearch, FaLayerGroup, FaCheckCircle
} from 'react-icons/fa';
import { useSelector } from 'react-redux';

export default function SchoolBatchDetailPage() {
  const { id } = useParams();
  ...
  const { data: studentData } = useGetAdminStudentsQuery({ search: studentSearch, schoolId: schoolId }, { skip: !studentSearch });
  ...
  // Fetch levels for the selected instrument
      ...
      <Modal show={showEnrollModal} onHide={() => setShowEnrollModal(false)} centered>
        ...
        <Modal.Body className="p-4">
          <Form.Group className="mb-4">
            <Form.Label className="small fw-bold">Search (My School Students)</Form.Label>
          ...
          <ListGroup variant="flush" className="border rounded">
            {foundStudents.map(student => (
              ...
              >
                <div>
                  ...
                  <div className="text-muted extra-small">{student.enrolmentNumber}</div>
                </div>
                <Button 
            ...
            {studentSearch && foundStudents.length === 0 && (
              <ListGroup.Item className="text-center py-4 text-muted small">No students found in your school.</ListGroup.Item>
            )}
            {!studentSearch && (
              <ListGroup.Item className="text-center py-4 text-muted small">Enter search term.</ListGroup.Item>
            )}
          </ListGroup>
        </Modal.Body>
      </Modal>
