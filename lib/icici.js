...
// FIX: Use base36 timestamp (saves ~4 chars) + 10 hex random chars = guaranteed uniqueness within 24 chars
// Layout: prefix(up to 3) + base36_timestamp(~8) + random(10) = ~21 chars, safely under 24
export const generateIciciMerchantTxnNo = (prefix = 'ORD') => {
  const safePrefix = prefix.slice(0, 3).toUpperCase();                      // max 3 chars
  const timestamp = Date.now().toString(36).toUpperCase();                  // ~8 chars (base36)
  const randomPart = crypto.randomBytes(5).toString('hex').toUpperCase();   // exactly 10 chars
  return `${safePrefix}${timestamp}${randomPart}`.slice(0, 24);
};
...
export const initiateIciciSaleWithFallback = async (payloadInput) => {
  ...
  for (const hashMode of getIciciHashRetryModes()) {
    // Always generate a fresh merchantTxnNo per attempt — ICICI rejects reused reference numbers
    const merchantTxnNo = generateIciciMerchantTxnNo(payloadInput.txnPrefix || 'ORD');
...
export const buildIciciRedirectUrl = (iciciResponse) => {
  ...
  const explicitTranCtx =
    findFirstStringByKeys(iciciResponse, ['tranCtx', 'tranctxt', 'transactionContext', 'txnCtx', 'context']) || null;

  if (explicitRedirectUri && explicitTranCtx) {
    const separator = explicitRedirectUri.includes('?') ? '&' : '?';
    return `${explicitRedirectUri}${separator}tranCtx=${explicitTranCtx}`;
  }
  ...
  const finalRedirectUri = explicitRedirectUri || rawRedirectUri;
  const finalTranCtx = explicitTranCtx || rawTranCtx;
