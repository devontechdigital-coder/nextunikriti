...
const formatPreferredSchedule = (enquiry = {}) => {
  ...
  return [preferredDays, preferredTimeSlots].filter(Boolean).join(' / ');
};

const formatEnrollmentSchedule = ({ preferredSchedule = [], preferredDays = [], preferredTimes = [] } = {}) => {
  const scheduleText = Array.isArray(preferredSchedule)
    ? preferredSchedule
      .filter((entry) => entry?.dayOfWeek && Array.isArray(entry.timeSlots) && entry.timeSlots.length)
      .map((entry) => `${entry.dayOfWeek}: ${entry.timeSlots.filter(Boolean).join(', ')}`)
      .join(' / ')
  ...
  if (scheduleText) return scheduleText;
  return [preferredDays.join(', '), preferredTimes.join(', ')].filter(Boolean).join(' / ');
};
...
export function buildTrialEnquiryEmailHtml({
  ...
  const preferredSchedule = formatPreferredSchedule(enquiry);
...
export function buildEnrollmentEmailHtml({
  ...
  const preferredScheduleText = formatEnrollmentSchedule({ preferredSchedule, preferredDays, preferredTimes });
  const rows = [
    ['Student', studentName],
    ['Course', courseTitle],
    ['Package', packageName],
    ...
    ['Preferred Schedule', preferredScheduleText],
  ].filter(([, value]) => value !== undefined && value !== null && String(value).trim());
